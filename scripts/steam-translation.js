 ...
 */
const getTranslationExcerpt = async (displayName, excerptFromJson, language) => {
    if (excerptFromJson) {
        return excerptFromJson;
    }

    let translationNotice = `This is a translation package for the ${displayName} mod.`;
    try {
...
const generateTranslationWorkshopTxt = async (language, locale, workshopMdPath, outputTxtPath) => {
    const { version, name, displayName } = getInfo();
    const translationWorkshopMetadata = await getTranslationWorkshopMetadata(language, locale);
    const { extracted, descriptionBbcode: generatedDescriptionBbcode } = await getLocalizedWorkshopDescription(
        workshopMdPath,
        locale,
        language
    );
    const descriptionBbcode = translationWorkshopMetadata.description || generatedDescriptionBbcode;
    const excerpt = await getTranslationExcerpt(displayName, translationWorkshopMetadata.excerpt, language);
